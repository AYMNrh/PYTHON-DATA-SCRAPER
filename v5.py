...
# Find the outer table by its class name
outer_table = soup.find('table', {'class': 'bordertb'})

# Check if outer table exists
if outer_table is not None:
    ...
    if inner_table is not None:
        ...
        headers = [header.text for header in inner_table.find_all('tr')[2].find_all('td')]
        rows = []
